                ...
                <div class="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm mb-8">
                    <div class="flex items-center justify-between mb-4">
                        ...
                        <div class="flex justify-end space-x-2">
                            <button onclick="app.printUnprintedOrdersAll()" class="text-xs bg-indigo-600 text-white px-3 py-1.5 rounded-md font-medium hover:bg-indigo-700 transition">
                                伝票印刷
                            </button>
                            <button onclick="app.loadAdminOrders()" class="text-xs bg-white border border-gray-200 text-gray-600 px-3 py-1.5 rounded-md font-medium hover:bg-gray-50 active:bg-gray-100 transition">
                                同期リフレッシュ
...
const app = {
        ...
        try {
            ...
            data.list.forEach(order => {
                ...
                const printButtonHtml = (order.printed_status === 1)
                    ?  `<button onclick='app.triggerReprint(${JSON.stringify(order).replace(/'/g, "&apos;")}, ${JSON.stringify(targetDate)})' 
                                class="bg-gray-100 text-gray-600 hover:bg-gray-200 px-3 py-1.5 rounded text-xs font-medium">
                            🔄 再印刷
                        </button>`
                    :  `<button onclick='app.printSingleOrderHtml(${JSON.stringify(order).replace(/'/g, "&apos;")}, ${JSON.stringify(targetDate)})' 
                ...
                html += `
                    <tr class="hover:bg-gray-50/50 transition ${isReceived ? 'bg-gray-50/40 text-gray-400' : ''}">
                        ...
                        <td class="p-3 px-2 text-right font-black text-gray-800 align-top">
                            ...
                            <div class="w-full mt-3">${printButtonHtml}</div>
                        </td>
                        <td class="p-3 px-2 text-center align-top">
    ...
    async printSingleOrderHtml(order, targetDate) {
        ...
        try {
            ...
            // 💡 バグ修正: 通信自体が失敗した場合（証明書未信頼・ネットワーク不通等）に
            // 何も表示されず「ボタンを押しても反応しない」ように見えていたため、
